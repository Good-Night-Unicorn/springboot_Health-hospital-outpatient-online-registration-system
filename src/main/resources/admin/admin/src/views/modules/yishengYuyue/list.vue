        ...
        <div v-if="showFlag">
                ...
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '开始挂号时间' : ''">
                        <el-date-picker
                                ...
                                v-model="searchForm.yishengYuyueTimeStartSearch"
                                type="datetime"
                                placeholder="开始时间">
                        </el-date-picker>
                    </el-form-item>
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '结束挂号时间' : ''">
                        <el-date-picker
                                value-format="yyyy-MM-dd HH:mm:ss"
                                v-model="searchForm.yishengYuyueTimeEndSearch"
                                type="datetime"
                                placeholder="结束时间">
                        </el-date-picker>
                    </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           ...
                           href="http://localhost:8080/jkyymenzhanzaixiangh/upload/yishengYuyueMuBan.xls"
                        >批量导入挂号数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('yishengYuyue','导入导出')"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="yishengYuyueTypes"
                                      header-align="center"
                                      label="挂号状态">
                        <template slot-scope="scope">
                            {{scope.row.yishengYuyueValue}}
                        </template>
                    </el-table-column>
            ...
            <el-button v-if="isAuth('yishengYuyue','订单') && scope.row.yishengYuyueTypes == 101 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-sold-out" size="mini" @click="refund(scope.row.id)">取消</el-button>
            <el-button v-if="isAuth('yishengYuyue','订单') && scope.row.yishengYuyueTypes == 103 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-reading" size="mini" @click="openCommentModel(scope.row.id)">评论</el-button>
            <el-button v-if="isAuth('yishengYuyue','订单') && scope.row.yishengYuyueTypes == 101 " type="primary" icon="el-icon-reading" size="mini"
    ...
    //当前表下拉框搜索条件
            ...
            json_fields: {
                //级联表字段
                     ...
                     '余额': 'newMoney',
                //本表字段
                     '挂号时间': "yishengYuyueTime",
                     '备注': "yishengYuyueText",
                     '挂号状态': "yishengYuyueValue",
                     '添加时间': "insertTime",
            },
        ...
        methods: {
            ...
            getDataList() {
                ...
                if (this.searchForm.yishengYuyueTimeStartSearch != '' && this.searchForm.yishengYuyueTimeStartSearch != undefined) {
                    params['yishengYuyueTimeStart'] = this.searchForm.yishengYuyueTimeStartSearch
                }
                if (this.searchForm.yishengYuyueTimeEndSearch != '' && this.searchForm.yishengYuyueTimeEndSearch != undefined) {
                    params['yishengYuyueTimeEnd'] = this.searchForm.yishengYuyueTimeEndSearch
                }
			...
			deliver(id){
				...
				_this.$confirm(`确定要完成吗?`, "提示", {
					...
					_this.$http({
						url: "yishengYuyue/deliver?id="+id,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							_this.$message({
			...
			commentback(id){
				...
				_this.$confirm(`确定要评论吗?`, "提示", {
					...
					_this.$http({
						url: "yishengYuyue/commentback?id="+_this.commentbackId+"&commentbackText="+_this.commentbackContent+"&yishengCommentbackPingfenNumber="+_this.yishengCommentbackPingfenNumber,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							_this.$message({
								...
								onClose: () => {
									...
									//情况评分
									_this.yishengCommentbackPingfenNumber = null
								}
							});
						} else {
							//清空操作数据id
